...
// (-3) invece che con golFatto (+3) � vedi app/lib/scoring.ts. La formula �
// gi� corretta per i NUOVI import; queste due action ricalcolano il
// fantavoto dei voti gi� salvati con la formula sbagliata (dryRun di default,
// apply=true per scrivere davvero) e rilanciano calculateScoresCore per le
// giornate coinvolte gi� importate, cos� anche punteggi/classifica gi�
    ...
    const newFantavoto = calculateFantavoto(
      {
        ...
        fantavoto: null, // se vote non � null, il fantavoto va sempre ricalcolato dagli eventi
...
export async function backfillGsrDryRun(prevState: string | null, formData: FormData) {
  ...
  if (diffs.length === 0) {
    return "? Nessun voto da correggere: nessun gol su rigore � stato scorato con la formula sbagliata.";
  }
