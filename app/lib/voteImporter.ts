...
export async function calculateScoresCore(matchdayId: number): Promise<void> {
  ...
  for (const match of matchesRes.rows) {
    ...
    // Gol mostrati (conversione punteggio->gol, null se disabilitata) e punti
    // partita (3/1/0): stessa logica di lega condivisa con le coppe, vedi
    // computeMatchOutcome in app/lib/scoring.ts per la spiegazione completa
    // (distacco minimo per vincere, bonus gol per fascia, soglia fissa 1 gol
    // per la squadra di casa).
    const { homeGoals, awayGoals, homePoints, awayPoints } = computeMatchOutcome(
      homeScore,
      awayScore,
      homeAdvantage,
      minWinMargin,
